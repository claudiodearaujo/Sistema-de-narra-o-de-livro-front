...
@Component({
  ...
  template: `
    <div class="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <!-- Modal -->
      <div class="relative bg-surface-card rounded-2xl shadow-xl max-w-md w-full max-h-[90vh] overflow-y-auto">
        <!-- Header -->
        <div class="flex items-center justify-between p-4 border-b border-surface-border">
          <h2 class="text-xl font-heading font-bold text-primary-800 dark:text-primary-200">Criar Grupo</h2>
          <button 
            (click)="close.emit()"
            class="p-2 text-secondary hover:text-primary-700 dark:hover:text-primary-300 rounded-lg hover:bg-secondary-100 dark:hover:bg-secondary-700">
        ...
        <form (ngSubmit)="onSubmit()" class="p-4 space-y-4">
          ...
          <div>
            ...
            <p class="text-xs text-secondary mt-1">{{ description.length }}/500 caracteres</p>
          </div>
              ...
              @for (option of privacyOptions; track option.value) {
                <label 
                  ...
                  [class]="privacy === option.value 
                    ...
                    : 'border-surface-border hover:bg-secondary-50 dark:hover:bg-secondary-800/50'">
                  <input 
                    type="radio"
                    name="privacy"
                    [value]="option.value"
                    ...
                    class="mt-1" />
                  <div>
                    <span class="font-medium text-primary-800 dark:text-primary-200">{{ option.label }}</span>
                    <p class="text-sm text-secondary">{{ option.description }}</p>
                  </div>
                </label>
              }
            </div>
          ...
          <div class="flex gap-3 pt-4">
            <button 
              type="button"
              (click)="close.emit()"
              class="flex-1 px-4 py-3 text-primary-700 dark:text-primary-300 bg-secondary-100 dark:bg-secondary-700 hover:bg-secondary-200 dark:hover:bg-secondary-600 rounded-lg font-medium transition-colors">
              Cancelar
            </button>
            <button 
              type="submit"
