    ...
    <div class="max-w-4xl mx-auto p-4">
      ...
      @if (loading()) {
        <div class="animate-pulse">
          <div class="h-48 bg-secondary-200 dark:bg-secondary-700 rounded-xl mb-6"></div>
          <div class="h-8 bg-secondary-200 dark:bg-secondary-700 rounded w-1/3 mb-4"></div>
      ...
      @if (!loading() && group()) {
        <!-- Header -->
        <div class="bg-surface-card rounded-xl shadow-sm overflow-hidden mb-6 border border-surface-border">
          <!-- Cover -->
          <div class="h-48 bg-linear-to-br from-primary-500 to-primary-700 relative">
            @if (group()!.coverUrl) {
          ...
          <div class="p-6">
            <div class="flex items-start justify-between gap-4">
              <div>
                <h1 class="text-2xl font-heading font-bold text-primary-800 dark:text-primary-200">{{ group()!.name }}</h1>
                @if (group()!.description) {
                  <p class="text-secondary mt-2">{{ group()!.description }}</p>
                }
                ...
                <div class="flex items-center gap-4 mt-4 text-sm text-secondary">
                  ...
                  <span class="px-2 py-1 bg-secondary-100 dark:bg-secondary-700 rounded text-xs">
                ...
                @if (group()!.isMember) {
                  @if (canManage()) {
                    <button 
                      ...
                      class="p-2 text-secondary hover:bg-secondary-100 dark:hover:bg-secondary-700 rounded-lg transition-colors">
                  ...
                  <button 
                    (click)="joinGroup()"
                    [disabled]="actionLoading()"
                    class="px-4 py-2 bg-primary-600 hover:bg-primary-700 disabled:bg-primary-400 text-white rounded-lg transition-colors">
                    @if (actionLoading()) {
                      Entrando...
                    } @else {
                      Participar
          ...
          <div>
            ...
            @if (canModerate() && group()!.isMember) {
              <button 
                ...
                class="w-full p-4 mb-4 border-2 border-dashed border-secondary-300 dark:border-secondary-600 rounded-xl text-secondary hover:border-primary-500 hover:text-primary-600 dark:hover:text-primary-400 transition-colors flex items-center justify-center gap-2">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            @if (campaigns().length > 0) {
              <div class="space-y-4">
                @for (campaign of campaigns(); track campaign.id) {
                  <a 
                    ...
                    class="block bg-surface-card rounded-xl p-6 shadow-sm hover:shadow-md transition-shadow border border-surface-border">
                    <div class="flex items-start justify-between gap-4">
                      <div class="flex-1">
                        <div class="flex items-center gap-2 mb-2">
                          <h3 class="font-semibold text-lg text-primary-800 dark:text-primary-200">{{ campaign.name }}</h3>
                          <span [class]="'px-2 py-0.5 text-xs rounded-full ' + getStatusClass(campaign.status)">
                            {{ getStatusLabel(campaign.status) }}
                          </span>
                        </div>
                        @if (campaign.description) {
                          <p class="text-secondary text-sm mb-3">{{ campaign.description }}</p>
                        }
                        ...
                        @for (book of campaign.books.slice(0, 3); track book.id) {
                          @if (book.book.coverUrl) {
                            <img [src]="book.book.coverUrl" [alt]="book.book.title" 
                              class="w-10 h-14 rounded border-2 border-white dark:border-secondary-800 object-cover" />
                          } @else {
                            <div class="w-10 h-14 rounded border-2 border-white dark:border-secondary-800 bg-linear-to-br from-primary-500 to-primary-700 flex items-center justify-center">
                              <span class="text-white text-xs font-bold">{{ book.book.title.charAt(0) }}</span>
                            </div>
                          }
                        }
                        @if (campaign.books.length > 3) {
                          <div class="w-10 h-14 rounded border-2 border-white dark:border-secondary-800 bg-secondary-100 dark:bg-secondary-700 flex items-center justify-center">
                            <span class="text-xs text-secondary">+{{ campaign.books.length - 3 }}</span>
                          </div>
                        }
                      </div>
                    </div>
              ...
              <div class="text-center py-12">
                ...
                <h3 class="text-lg font-heading font-medium text-primary-800 dark:text-primary-200 mb-2">Nenhuma campanha ainda</h3>
            ...
            @if (membersLoading()) {
              <div class="space-y-3">
                @for (i of [1, 2, 3, 4]; track i) {
                  <div class="flex items-center gap-3 p-4 bg-surface-card rounded-xl animate-pulse border border-surface-border">
                    ...
                    <div class="flex-1 space-y-2">
                      ...
                      <div class="h-3 bg-secondary-200 dark:bg-secondary-700 rounded w-1/6"></div>
                    </div>
                  </div>
                }
              </div>
              ...
              <div class="space-y-2">
                @for (member of members(); track member.id) {
                  <div class="flex items-center gap-4 p-4 bg-surface-card rounded-xl border border-surface-border">
                    <!-- Avatar -->
                    @if (member.user.avatar) {
                      <img [src]="member.user.avatar" [alt]="member.user.name" class="w-12 h-12 rounded-full object-cover" />
                    } @else {
                      <div class="w-12 h-12 rounded-full bg-linear-to-br from-primary-500 to-primary-700 flex items-center justify-center">
                    ...
                    <div class="flex-1 min-w-0">
                      <a [routerLink]="['/social/profile', member.user.username]" class="font-medium text-primary-800 dark:text-primary-200 hover:text-primary-600 dark:hover:text-primary-400">
                        {{ member.user.name }}
                      </a>
                    ...
                    @if (canManage() && member.role !== 'OWNER' && member.user.id !== currentUserId()) {
                      <div class="relative">
                        <button 
                          ...
                          class="p-2 text-secondary hover:bg-secondary-100 dark:hover:bg-secondary-700 rounded-lg">
                          <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 5v.01M12 12v.01M12 19v.01M12 6a1 1 0 110-2 1 1 0 010 2zm0 7a1 1 0 110-2 1 1 0 010 2zm0 7a1 1 0 110-2 1 1 0 010 2z" />
                          </svg>
                        </button>
                        @if (openMemberMenu() === member.id) {
                          <div class="absolute right-0 mt-2 w-48 bg-surface-card rounded-lg shadow-lg border border-surface-border z-10">
                            @if (member.role !== 'ADMIN') {
                              <button 
                                (click)="updateRole(member, 'ADMIN')"
                                class="w-full px-4 py-2 text-left text-sm hover:bg-secondary-50 dark:hover:bg-secondary-700">
                                Promover a Admin
                              </button>
                            }
              ...
              @if (hasMoreMembers()) {
                <div class="mt-4 text-center">
                  <button 
                    ...
                    class="px-4 py-2 text-primary-600 dark:text-primary-400 hover:bg-primary-50 dark:hover:bg-primary-900/20 rounded-lg">
                    Carregar mais
                  </button>
                </div>
              }
      ...
      @if (!loading() && !group()) {
        <div class="text-center py-12">
          <h2 class="text-xl font-heading font-bold text-primary-800 dark:text-primary-200 mb-2">Grupo não encontrado</h2>
          <p class="text-secondary mb-4">O grupo que você procura não existe ou foi removido.</p>
